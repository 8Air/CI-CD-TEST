...
void do_jpeg() 
{
	...
	if (stat("/xport/camdata/cap.jpg",&fstat)!=0) usleep(15000);
	if (stat("/xport/camdata/cap.jpg",&fstat)!=0) usleep(30000);
	if (stat("/xport/camdata/cap.jpg",&fstat)!=0) usleep(60000);
	if (stat("/xport/camdata/cap.jpg",&fstat)!=0) usleep(100000);
	if (stat("/xport/camdata/cap.jpg",&fstat)!=0) usleep(100000);
	if (stat("/xport/camdata/cap.jpg",&fstat)!=0) usleep(100000);
	if (stat("/xport/camdata/cap.jpg",&fstat)!=0) usleep(100000);
	if (stat("/xport/camdata/cap.jpg",&fstat)!=0) usleep(100000);
	if (stat("/xport/camdata/cap.jpg",&fstat)!=0) usleep(100000);
        ...
        if (stat("/xport/camdata/cap.jpg",&fstat)==0) {
		jpg_file=fopen("/xport/camdata/cap.jpg", "rb");
		rc = fread(jpg_buffer, 1, MAX_JPG_SZ, jpg_file);
		jpg_size=rc;
		fclose(jpg_file);
		remove("/xport/camdata/cap.jpg");
	} else {

		jpg_file=fopen("/xport/apptest-camera/cam.jpg", "rb");
		rc = fread(jpg_buffer, 1, MAX_JPG_SZ, jpg_file);
		jpg_size=rc;
		fclose(jpg_file);
//		printf("READ BK JPG\n");
