...
static T_DjiReturnCode DjiTest_DataSubscriptionReceiveQuaternionCallback(uint8_t *data, uint16_t dataSize,
...
//    system("echo 1 > /sys/class/gpio/gpio11/value");
    ...
    writefl(takePhoto,0, false, false, 0);
    x_InitPT = true;
...
static T_DjiReturnCode SetFocusMode(E_DjiCameraFocusMode mode)
{
    ...
    USER_LOG_INFO(">>> SetFocusMode set focus mode:%d", mode);
    writefl(infoFocusFile, mode ? focus_dec[1] : focus_dec[0], true, false, 0);
...
static T_DjiReturnCode SetFocusTarget(T_DjiCameraPointInScreen target)
{
    ...
    if (s_cameraFocusMode == DJI_CAMERA_FOCUS_MODE_AUTO) {
        writefl(doFocus, 0, false, false, 0);
...
static void *UserCamera_Task(void *arg)
{
    ...
    int a;
    struct stat fstat;
    const uint32_t timeToSleep = 1000;
    while (1)
    {
        int value = readfl(setFocusFile, true);
        if (value == -1) {
            Osal_TaskSleepMs(timeToSleep);
            continue;
        }

        int32_t index = findIndex(focus_dec, sizeof(focus_dec) / sizeof(int), value);
        s_cameraFocusMode = (focus_dec[index] == 0x0002 ) ? DJI_CAMERA_FOCUS_MODE_AUTO :
                            DJI_CAMERA_FOCUS_MODE_MANUAL;
/*	    s_cameraState.currentPhotoShootingIntervalTimeInSeconds =
				x_userIntervalTimeSeconds != 0.f ?
					x_userIntervalTimeSeconds :
                                                                         ...
                                                                         T_DjiDataTimestamp *timestamp)
{
	...
	T_DjiQuaternion4f *quaternion = (T_DjiQuaternion4f *) data;
	
	USER_UTIL_UNUSED(dataSize);
	DjiLogger_UserLogOutput(DJI_LOGGER_CONSOLE_LOG_LEVEL_INFO,"receive quaternion data.");
	DjiLogger_UserLogOutput(DJI_LOGGER_CONSOLE_LOG_LEVEL_INFO,"timestamp: millisecond %u microsecond %u.", timestamp->millisecond,
	                        timestamp->microsecond);
	DjiLogger_UserLogOutput(DJI_LOGGER_CONSOLE_LOG_LEVEL_INFO,"quaternion: %f %f %f %f.", quaternion->q0, quaternion->q1, quaternion->q2, quaternion->q3);
	return DJI_RETURN_CODE_OK;
}

T_DjiReturnCode DjiTest_GPSInit(void) {
...
//    s_cameraState.shootingState=DJI_CAMERA_SHOOT_PHOTO_MODE_SINGLE;
	...
	printf("%d\r\n", __LINE__);
	DjiLogger_UserLogOutput(DJI_LOGGER_CONSOLE_LOG_LEVEL_INFO,">>> %d", __LINE__);
	...
	DjiLogger_UserLogOutput(DJI_LOGGER_CONSOLE_LOG_LEVEL_INFO,">>> %d", __LINE__);
	if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
		DjiLogger_UserLogOutput(DJI_LOGGER_CONSOLE_LOG_LEVEL_ERROR,
								"camera register tap zoom handler error:0x%08llX",
								returnCode);
	...
	if (Osal_TaskCreate("user_camera_task", &s_userCameraThread, 4096, NULL, UserCamera_Task)
	...
	DjiLogger_UserLogOutput(DJI_LOGGER_CONSOLE_LOG_LEVEL_INFO,">>> %d", __LINE__);
	if (Osal_TaskCreate("user_interval_shoot_task",&s_userIntervalShootingThread,4096,NULL,UserIntervalShoot_Task)
        != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
