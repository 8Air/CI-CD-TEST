...
static void *UserCameraMedia_SendVideoTask(void *arg)
{
 ...
 //       DjiLogger_UserLogOutput(DJI_LOGGER_CONSOLE_LOG_LEVEL_INFO,"Frame info: read %d ms, convert %d ms, compress %d ms, FPS: %d, sending size: %d", timeRead, timeConvert, timeCompress, fps, dataLength);
        ...
        while (dataLength - lengthOfDataHaveBeenSent) {
            ...
            DjiStat = DjiPayloadCamera_SendVideoStream((const uint8_t *) h264_buffer_ready + lengthOfDataHaveBeenSent,
...
T_DjiReturnCode  DjiTest_CameraMediaInit(void)
{
   ...
   if (DjiPayloadCamera_SetVideoStreamType(DJI_CAMERA_VIDEO_STREAM_TYPE_H264_CUSTOM_FORMAT)
    ...
    if (Osal_TaskCreate("user_camera_media_task",&s_userSendVideoThread,2048, NULL, UserCameraMedia_SendVideoTask
                            ) != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
    ...
    Osal_TaskCreate("user_camera_media_read_task", &ts_ReadThread, 2048, NULL,UserCameraMedia_ReadTask);
    Osal_TaskCreate("user_camera_media_convert_task",&ts_ConvertThread,2048,UserCameraMedia_ConvertTask,NULL);
    Osal_TaskCreate("user_camera_media_compress_task", &ts_CompressThread,2048, UserCameraMedia_CompressTask,NULL);
